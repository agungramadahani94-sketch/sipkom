...
<body class="font-sans bg-paper text-ink overflow-x-hidden">
        ...
        <div class="max-w-[1100px] mx-auto px-6 h-16 flex items-center justify-between">
            ...
            <div class="hidden md:flex gap-1">
                @foreach ([['#', 'Home'], ['#about', 'About'], ['#fitur', 'Fitur']] as [$href, $label])
                    <a href="{{ $href }}" class="font-syne font-bold text-[12px] tracking-[.08em] uppercase px-3.5 py-2 text-white no-underline border-2 border-transparent hover:bg-yellow hover:border-ink hover:text-ink hover:shadow-hard-sm transition-all">
                        {{ $label }}
                    </a>
                @endforeach
                <a href="{{ route('login') }}" class="font-syne font-bold text-[12px] tracking-[.08em] uppercase px-3.5 py-2 text-white no-underline border-2 border-transparent hover:bg-yellow hover:border-ink hover:text-ink hover:shadow-hard-sm transition-all">
                    Login
                </a>
            </div>
        ...
        <div class="max-w-[1100px] mx-auto px-6 py-16 flex flex-col lg:flex-row items-center gap-10">
            ...
            <div class="reveal flex-1">
                ...
                <div class="flex flex-wrap gap-3">
                    <a href="#about" class="font-syne font-bold text-[13px] uppercase tracking-[.08em] px-7 py-3.5 border-[2.5px] border-ink shadow-hard bg-yellow text-ink no-underline inline-flex items-center gap-2 hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-hard-sm transition-all">
                        <i class="ti ti-info-circle"></i> Tentang Kami
                    </a>
                    <a href="#fitur" class="font-syne font-bold text-[13px] uppercase tracking-[.08em] px-7 py-3.5 border-[2.5px] border-ink shadow-hard bg-white text-ink no-underline inline-flex items-center gap-2 hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-hard-sm transition-all">
    ...
    <div class="bg-ink border-y-[2.5px] border-ink overflow-hidden py-[11px]">
        <div class="ticker-anim flex w-max">
            @foreach (['Manajemen Aset','Peminjaman Online','Dashboard Real-time','Data Anggota','Laporan Aktivitas','Autentikasi Aman','Notifikasi Otomatis','Multi Divisi','Manajemen Aset','Peminjaman Online','Dashboard Real-time','Data User','Laporan Aktivitas','Notifikasi Otomatis','Autentikasi Aman','Multi Divisi'] as $item)
                <div class="flex items-center gap-2.5 px-7 text-white font-mono text-[11px] uppercase whitespace-nowrap">
                    <div class="w-[5px] h-[5px] rounded-full bg-yellow flex-shrink-0"></div>
                    {{ $item }}
                </div>
            @endforeach
        </div>
    ...
    <section class="bg-ink2">
        <div class="reveal max-w-[1100px] mx-auto grid grid-cols-2 lg:grid-cols-4">
            @foreach ([
                ['ti-users', $activeMembers, 'Anggota Aktif'],
                ['ti-clipboard-list', $loanCount, 'Peminjaman'],
                ['ti-check', $availableCount, 'Aset Tersedia'],
                ['ti-laptop', $assetCount, 'Aset Tercatat'],
            ] as $i => [$icon, $value, $label])
                <div class="stat relative overflow-hidden px-5 py-11 flex flex-col items-center gap-2.5 {{ $i < 3 ? 'border-r border-white/10' : '' }}">
                    <div class="w-11 h-11 border border-white/25 flex items-center justify-center text-yellow text-xl">
                        <i class="ti {{ $icon }}"></i>
                    </div>
                    <div class="font-syne font-extrabold text-[34px] text-white leading-none">{{ $value }}</div>
                    <div class="font-mono text-[10px] uppercase text-white/50 text-center">{{ $label }}</div>
                </div>
            @endforeach
        </div>
    </section>
    ...
    <footer class="bg-ink border-t-[3px] border-yellow">
        ...
        <div class="max-w-[1100px] mx-auto px-6 py-8 flex flex-wrap items-center justify-between gap-5">
            ...
            <div class="flex items-center gap-1 flex-wrap">
                <a href="#" class="font-syne font-bold text-[11px] uppercase px-3.5 py-[7px] border border-white/12 text-white/50 hover:bg-yellow hover:text-ink transition-all no-underline">
